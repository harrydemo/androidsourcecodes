...
public class QueryIDCard extends BaseActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setView(R.layout.query_idcard);
		SetTitle(getString(R.string.number_id_info));
	...
	private void setListener() {
		query_btn.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				boolean hasNet = new InternetHelper(QueryIDCard.this)
				...
				if (q == null || q.length() == 0) {
					showToast(getString(R.string.input_idcard_no_desc));
				} else {
					queryData(q);
				}
			}
	...
	Handler hander = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			switch (msg.what) {
			case QUERY_MSG:
				IDCardInfo pi = (IDCardInfo) msg.obj;
				String ret = getString(R.string.query_idcard_result_template,pi.getCode(), pi.getBirthday(),pi.getGender(),pi.getLocation());
	...
	protected void HandleTitleBarEvent(int buttonId) {
		...
		case 0:
			...
			break;
		case 1:
			Intent intent = new Intent(getApplicationContext(), SmallToolsActivity.class);
