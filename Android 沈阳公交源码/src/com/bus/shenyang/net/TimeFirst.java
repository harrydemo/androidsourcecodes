...
public class TimeFirst extends MapActivity implements UpdatePointsNotifier {


	private String ststop;
	private String enstop;
	private BMapManager mapManagerTimeFirst;
	...
	public void onCreate(Bundle savedInstanceState){
		...
		setContentView(R.layout.timefirst);
		...
		listView.setOnItemClickListener(new TimeFirstListViewOnItemClickListener());
	      
		Bundle bundle = this.getIntent().getExtras();
		ststop = bundle.getString("ststop1");
		enstop = bundle.getString("enstop1");
		
		mapManagerTimeFirst = new BMapManager(getApplication());
		mapManagerTimeFirst.init("64174E2288F6C683C4CDA6094DE98AEDAFB577C8",null);
		super.initMapActivity(mapManagerTimeFirst);
		...
		mSearch.init(mapManagerTimeFirst, new MyTimeFirstSearchListener());
	...
	protected void onPause() {
		if (mapManagerTimeFirst != null) {
			mapManagerTimeFirst.stop();
		}
		super.onPause();
	}
	@Override
	protected void onResume() {
		...
		super.onResume();
		pd = ProgressDialog.show(TimeFirst.this, "请稍候", "正在连接服务器...", true, true);
		listView = (ListView) findViewById(R.id.listView);
		mAdapter = new BusAdapter(getLayoutInflater());
		listView.setAdapter(mAdapter);
		
		System.out.println("/////////");
		System.out.println("TimeFirst ststop=" + ststop);
		System.out.println("TimeFirst enstop=" + enstop);
		System.out.print("BBBBBBBBBBBBB");
		mapManagerTimeFirst = new BMapManager(getApplication());
		mapManagerTimeFirst.init("64174E2288F6C683C4CDA6094DE98AEDAFB577C8",null);
		mapManagerTimeFirst.start();
		mSearch = new MKSearch();
		mSearch.init(mapManagerTimeFirst, new MyTimeFirstSearchListener());
		MKPlanNode stNode = new MKPlanNode();
		stNode.name = ststop;
	...
	public class TimeFirstListViewOnItemClickListener implements
	OnItemClickListener {
...
public void onItemClick(AdapterView<?> parent, View view, int position,
		long id) {
	AppConnect.getInstance(TimeFirst.this).getPoints(TimeFirst.this);
	int a=flag;
	if(a>=amount)
	{						
		AppConnect.getInstance(TimeFirst.this).spendPoints(amount, TimeFirst.this);
	state=true;
	}else{
		System.out.println("1111111111111111");
		AppConnect.getInstance(TimeFirst.this).showOffers(TimeFirst.this);
		String str = "您的积分余额不足1分不能使用相关功能,余额为"+flag+"请获取积分";
		Toast.makeText(TimeFirst.this
	...
	Intent intent = new Intent(TimeFirst.this, Detailed.class);
	...
	public class MyTimeFirstSearchListener implements MKSearchListener {
		...
		public void onGetTransitRouteResult(MKTransitRouteResult res, int error) {
			...
			if (error != 0 || res == null) {
				Toast.makeText(TimeFirst.this, "抱歉，未找到结果",
