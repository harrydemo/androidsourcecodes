...
public class RtspVideoRecorder extends IMediaPlayer.Stub implements Camera.PreviewCallback {
    ...
    private Thread captureThread = new Thread() {
        ...
        private byte[] encoded(byte[] pframeData,long pLength)
        {
        	 byte[] encodedFrame;
        	if (selectedVideoCodec.getCodecName().equalsIgnoreCase(H264Config.CODEC_NAME)) {
        		encodedFrame = NativeH264Encoder.EncodeFrame(pframeData, pLength);
              //  return NativeH264Encoder.getLastEncodeStatus();
            } else {
            	encodedFrame = NativeH263Encoder.EncodeFrame(pframeData, pLength);
               // return 0;
            }
        ...
        private int getLastEncodeStatus()
        {
        	if (selectedVideoCodec.getCodecName().equalsIgnoreCase(H264Config.CODEC_NAME)) {
...
//            }
            ...
            while (started) {
                ...
                //±àÂëÂß¼­
                if(RtpSender.IsIntoUser)
                {
             	   RtpSender.IsIntoUser=false;
             	   encodedFrame=encoded(new byte[frameData.length], encoderTs);
                }
                else if(frameCount>200)
                {
               	 ...
               	 //Çå¿Õ±àÂë»º´æ
               	 encodedFrame=encoded(new byte[frameData.length], encoderTs);
               	 encodedFrame=encoded(frameData, encoderTs);
                }else
                {
                	encodedFrame=encoded(frameData, encoderTs);
                }
              //±àÂëÂß¼­
                ...
                if ((encodeResult == 0 || encodeResult == 6) && encodedFrame.length > 0) {
                    ...
                    System.out.println("RtpVideoRecorder: encodedFrame.length == "+encodedFrame.length);
                }
