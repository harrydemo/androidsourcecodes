...
//	log.wf("==========fbreader===","===tagString:%s==",&tagString);
...
void OEBBookReader::generateTOC() {
	...
	if (!myNCXTOCFileName.empty()) {
		//log.wf("=====fbreader====","===myNCXTOCFileName===%s===",&myNCXTOCFileName);
		NCXReader ncxReader(myModelReader);
		if (ncxReader.readDocument(ZLFile(myFilePrefix + myNCXTOCFileName))) {
		//	log.wf("=====fbreader====","===ZLFile===%s===",&(myFilePrefix + myNCXTOCFileName));
...
//					log.wf("=========fbreader===","=====index:%d===",index);
//							log.wf("=========fbreader===","=====level:%d===",level);
					while (level > point.Level) {
//						log.wf("=========fbreader===","=====point.Level:%d===",point.Level);
...
//					log.wf("=========fbreader===","=====text:%s===",&(point.Text));
