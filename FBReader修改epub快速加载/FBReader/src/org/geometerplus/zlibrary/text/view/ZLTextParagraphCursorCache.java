...
class ZLTextParagraphCursorCache
{
	...
	public static void put(ZLTextModel model, int index,
			ZLTextParagraphCursor cursor)
	{
//		L.l("========" + index + "===" + model.getId() + "====cursor:"
//				+ cursor.getParagraphLength());
//		L.l("========" + cursor.Index+"  element:"+model.getParagraph(cursor.Index).iterator().getType());
		ourMap.put(new Key(model, index),
				new WeakReference<ZLTextParagraphCursor>(cursor));
	}

	public static ZLTextParagraphCursor get(ZLTextModel model, int index)
	{
		WeakReference<ZLTextParagraphCursor> ref = ourMap.get(new Key(model,
				index));
