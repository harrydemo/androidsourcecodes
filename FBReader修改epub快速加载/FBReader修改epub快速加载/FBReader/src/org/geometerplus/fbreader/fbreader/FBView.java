...
public final class FBView extends ZLTextView {
	...
	public boolean onFingerSingleTap(int x, int y) {
		...
		myReader.doActionWithCoordinates(
				getZoneMap()
	...
	public boolean onFingerDoubleTap(int x, int y) {
		...
		myReader.doActionWithCoordinates(
				getZoneMap().getActionByCoordinates(x, y, myContext.getWidth(),
						myContext.getHeight(), TapZoneMap.Tap.doubleTap), x, y);
		return true;
	}
	...
	public boolean onFingerRelease(int x, int y) {
		...
		if (isFlickScrollingEnabled()) {
			int pageTurn = myReader.getViewWidget().startAnimatedScrolling(
					x,
					y,
					ScrollingPreferences.Instance().AnimationSpeedOption
							.getValue());

			// 对于判断如果不能翻页了，则添加新的一段
			L.l("========pageTurn" + pageTurn);
			if (pageTurn > 0)
				myReader.reloadEpubBook(0);
			// 对于判断如果不能翻页了，则添加新的一段
			return true;
		}
	...
	public boolean onTrackballRotated(int diffX, int diffY) {
		...
		ZLTextRegion region = getSelectedRegion();
		final ZLTextRegion.Filter filter = (region != null && region.getSoul() instanceof ZLTextWordRegionSoul)
				|| myReader.NavigateAllWordsOption.getValue() ? ZLTextRegion.AnyRegionFilter
	...
	public ZLColor getTextColor(ZLTextHyperlink hyperlink) {
		...
		case FBHyperlinkType.INTERNAL:
			return myReader.Model.Book.isHyperlinkVisited(hyperlink.Id) ? profile.VisitedHyperlinkTextOption
	...
	private class Footer implements FooterArea {
		...
		public synchronized void paint(ZLPaintContext context) {
			...
			context.setFont(reader.FooterFontOption.getValue(),
			...
			final int gaugeInternalRight = left + lineWidth
					+ (int) (1.0 * myGaugeWidth * pagesProgress / bookLength);
