...
public abstract class SRPGScreen extends Screen implements SRPGType, Runnable {
	...
	private LImage cursor, messageImage;
	...
	public SRPGScreen(String fileName, LImage img) {
	...
	public SRPGScreen(String fileName, LImage img, int row, int col) {
		this(null, fileName, img, row, col);
	}

	public SRPGScreen(SRPGFieldElements elements, String fileName, LImage img,
			int row, int col) {
	...
	protected void makeEmulatorButton(String f, int x, int y) {
		Bitmap img = GraphicsUtils.loadBitmap(f);
	 ...
	 * @param y
	 */
	protected void makeEmulatorButtons(Bitmap on, Bitmap un, int x, int y) {
	...
	private LImage createTempImage() {
		if (messageImage == null) {
			messageImage = LImage.createImage(getWidth() - 40,
					getHeight() / 2 - 20, true);
			LGraphics g = messageImage.getLGraphics();
			g.setColor(0, 0, 0, 125);
			g.fillRect(0, 0, messageImage.getWidth(), messageImage.getHeight());
			g.dispose();
		}
		return messageImage;
	}
	...
	private void afterCheck() {
		...
		for (int j = 0; j < srpgActors.size(); j++) {
			...
			if (status.exp != -100) {
				...
				setEffect(new SRPGNumberEffect(actor.getPosX() * tileWidth,
						actor.getPosY() * tileHeight, LColor.red, "Level Down!"));
			}
		...
		for (int i = 0; i < srpgActors.size(); i++) {
			...
			if (result != null) {
				...
				g.drawStyleString(result, (actor.drawX() - size) - cam_x, actor
						.drawY()
						- 3 - cam_y, LColor.black, LColor.white);
			}
		}
	 ...
	 * @param y
	 */
	private final void drawCube(LGraphics g, LColor color, int x, int y) {
		int x1 = (x * tileWidth - cam_x) + 1;
		int y1 = (y * tileHeight - cam_y) + 1;
		if (isGridAlpha) {
			int c = g.getColorRGB();
			g.setColorARGB32(color.getARGB());
	...
	public final void setMessageCharacter(LImage img, int x, int y) {
	...
	public final void setHelper(String[] message, int x, int y, int w, int h,
		...
		srpgHelper.setBackground((LImage) null);
		if (flag) {
			srpgHelper.printWait(this);
		} else {
			srpgHelper.messageWait(this);
	...
	public final void setHelperCharacter(LImage img, int x, int y) {
		if (srpgHelper != null) {
	 ...
	 * @return
	 */
	public final void onTouchDown(final LTouch e) {
	 ...
	 * @return
	 */
	public final void onTouchUp(LTouch e) {
		if (!isEventLoop || isSrpgTouchLock) {
			return;
		}
		onUp(e);
	 ...
	 * @param e
	 */
	public final void onTouchMove(final LTouch e) {
	...
	public void setMessageImage(LImage img) {
		this.messageImage = img;
	}
