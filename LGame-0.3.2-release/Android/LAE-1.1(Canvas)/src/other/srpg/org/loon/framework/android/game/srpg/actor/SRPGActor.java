...
public class SRPGActor implements LRelease {
	 ...
	 * @return
	 */
	public LImage[] getAnimationImages(final AnimationHelper o, final int d) {
		LImage[] result = null;
		switch (d) {
		// 向下移动
	...
	public LImage getImage() {
		LImage[] img = getAnimationImages(move, SRPGType.MOVE_DOWN);
		if (img != null) {
	...
	private void drawActor(LGraphics g, int x, int y) {
		...
		LImage img = actionImage[actionCount];
		// 移动中(此处可能会修改加入特殊效果)
		if (isMoving) {
			g.drawImage(img, x + (tileWidth - img.getWidth()) / 2, y
					+ (tileHeight - img.getHeight()) / 2);
			// 角色行动完毕
		} else if (isEnd) {
			g.drawImage(endImage, x + (tileWidth - img.getWidth()) / 2, y
					+ (tileHeight - img.getHeight()) / 2);
			// 其它情况
		} else {
			g.drawImage(img, x + (tileWidth - img.getWidth()) / 2, y
