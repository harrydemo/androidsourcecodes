...
public class SRPGField implements LRelease {
	...
	private LImage bigImageMap, tempFieldImage;
	...
	public Bitmap getPosImage(int x, int y) {
		if (x < 0 || y < 0 || posX >= width || posY >= height) {
			return null;
		}
		switch (fieldMode) {
		...
		case SRPGType.FIELD_BIGMAP:
			if (bigImageMap == null) {
				...
				LImage tmp = tempFieldImage;
				tempFieldImage = GraphicsUtils.drawClipImage(bigImageMap,
						tileWidth, tileHeight, x * tileWidth, y * tileHeight);
				if (tmp != null) {
					tmp.dispose();
					tmp = null;
				}
				return tempFieldImage.getBitmap();
			}
	...
	public void draw(LGraphics g, int x, int y, int w, int h) {
		...
		case SRPGType.FIELD_NORMAL:
			...
			x2 = x % tileWidth;
			y2 = y % tileHeight;
			int maxY = h / tileHeight + (h % tileHeight + limitHeight)
					/ tileHeight + (y % tileHeight + limitHeight) / tileHeight;
			int maxX = w / tileWidth + (w % tileWidth + limitWidth) / tileWidth
			...
			for (int ny = 0; ny < maxY; ny++) {
				for (int nx = 0; nx < maxX; nx++) {
					...
					SRPGFieldElement element = battleList
							.getBattleElement(index);
					if (element == null) {
						continue;
					}
					g.drawBitmap(element.image, nx * tileWidth - x2, ny
							* tileHeight - y2);
				}
			}
			break;
		case SRPGType.FIELD_BIGMAP:
			g.drawBitmap(bigBitmapMap, -x, -y);
			break;
		case SRPGType.FIELD_BLEND:
			x1 = x / tileWidth;
			y1 = y / tileHeight;
			x2 = x % tileWidth;
			y2 = y % tileHeight;
			maxY = h / tileHeight + (h % tileHeight + limitHeight) / tileHeight
	...
	public synchronized void setBigImageMap(String fileName) {
		setBigImageMap(GraphicsUtils.loadNotCacheImage(fileName));
	}

	public synchronized void setBigImageMap(LImage img) {
		if (img != null) {
			if (img.getWidth() == drawWidth && img.getHeight() == drawHeight) {
				this.bigImageMap = img;
				if (bigImageMap.getConfig() != Config.RGB_565) {
					Bitmap temp = bigImageMap.getBitmap();
					Bitmap newBitmap = temp.copy(Config.RGB_565, false);
					try {
						temp.recycle();
						temp = null;
					} catch (Exception e) {
					}
					this.bigBitmapMap = newBitmap;
					this.bigImageMap.setBitmap(newBitmap);
				} else {
					this.bigBitmapMap = img.getBitmap();
				}
			} else {
				this.bigImageMap = GraphicsUtils.drawClipImage(img, drawWidth,
						drawHeight, 0, 0);
				if (bigImageMap.getConfig() != Config.RGB_565) {
					Bitmap temp = bigImageMap.getBitmap();
					Bitmap newBitmap = temp.copy(Config.RGB_565, false);
	...
	public void dispose() {
		...
		if (bigBitmapMap != null) {
			bigBitmapMap.recycle();
			bigBitmapMap = null;
		}
		if (chips != null) {
			chips = null;
