...
public final class GLEx implements LTrans {
	 ...
	 * @param c
	 */
	public void drawOESImage(LImage image, float dx1, float dy1, float dx2,
			float dy2, float sx1, float sy1, float sx2, float sy2, GLColor c) {
		if (isClose || image == null || image.isClose()) {
			return;
		}
		drawOESTexture(image.getTexture(), dx1, dy1, dx2, dy2, sx1, sy1, sx2,
				sy2, c);
	}
		...
		if (oesOn) {
			...
			glTex2DEnable();
			{
				...
				if (LSystem.scaleWidth == 1 && LSystem.scaleHeight == 1) {
					...
					gl11ex
							.glDrawTexfOES(dx1 * LSystem.scaleWidth,
									(viewPort.height - dy2 - dy1)
											* LSystem.scaleHeight, 0, dx2
											* LSystem.scaleWidth, dy2
											* LSystem.scaleHeight);
				}
	...
	private final static void putRectangle(LTexture texture, float srcX,
		...
		float xOff = ((srcX / texture.width) * texture.widthRatio)
				+ texture.xOff;
		float yOff = ((srcY / texture.height) * texture.heightRatio)
				+ texture.yOff;
	...
	private void drawString(final String string, float x, float y,
			float rotation, GLColor c, boolean check) {
	 ...
	 */
	private void drawString(final String string, float x, float y,
			float rotation, GLColor c, boolean check, boolean notEnglish) {
				...
				glTex2DARRAYEnable();
				{
					...
					if (vboOn) {
						...
						for (int i = 0; i < size; i++) {
							...
							gl11.glTexCoordPointer(2, GL11.GL_FLOAT, 8,
									chars[i] * 32);
							gl11.glDrawArrays(GL11.GL_TRIANGLE_STRIP, 0, 4);
	...
	public void drawStyleString(String message, float x, float y, GLColor c1,
			GLColor c2) {
		if (isClose) {
			return;
		}
	...
	final void glTexEnvfModulateColor(float r, float g, float b, float a,
			boolean update) {
		try {
			gl10.glTexEnvf(GL10.GL_TEXTURE_ENV, GL10.GL_TEXTURE_ENV_MODE,
					GL10.GL_MODULATE);
			if (update) {
				setColor(r, g, b, a);
			}
	...
	public void begin2DTex() {
		if (vboOn) {
			gl.glLoadIdentity();
			if (!gl11.glIsEnabled(GL10.GL_TEXTURE_2D)) {
				gl11.glEnable(GL10.GL_TEXTURE_2D);
				preTex2d = false;
			}
			if (gl11.glIsEnabled(GL10.GL_LIGHTING)) {
	...
	public void end2DTex() {
		...
		if (preLight) {
			gl11.glEnable(GL10.GL_LIGHTING);
		} else {
			gl11.glDisable(GL10.GL_LIGHTING);
		}
		if (preCull) {
			gl11.glEnable(GL10.GL_CULL_FACE);
		} else {
			gl11.glDisable(GL10.GL_CULL_FACE);
		}
	}
