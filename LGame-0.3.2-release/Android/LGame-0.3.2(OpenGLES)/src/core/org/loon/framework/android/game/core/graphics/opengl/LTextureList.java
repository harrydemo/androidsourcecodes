...
public class LTextureList implements LRelease {
	...
	public LTexture loadTexture(String name) {
		...
		if (data.mask != null) {
			...
			img = LTextures.loadTexture(data.ref,
					data.scaleType == 0 ? Format.DEFAULT : Format.LINEAR).get();
		}
	...
	private int blendMode = GL.MODE_NONE;
	...
	public int add(String res, Format format, float x, float y) {
		return add(new LTextureObject(LTextures.loadTexture(res, format).get(),
				x, y));
	}
	...
	public void draw(GLEx g, int minX, int minY, int maxX, int maxY) {
		...
		synchronized (values) {
			...
			for (int i = 0; i < count; i++) {
				...
				g.drawBatch(texture, tex2d.x, tex2d.y);
			}
			g.endBatch();
			if (alpha > 0 && alpha < 1) {
				g.setAlpha(1f);
