...
public class LProcess {
	...
	private Screen currentControl, tmpControl;
	...
	public LProcess(LGameAndroid2DView scene, View view, int width, int height) {
		this.width = width;
		this.height = height;
		this.scene = scene;
	...
	public int getFPS() {
		if (scene != null) {
			return (int) scene.getCurrentFPS();
		}
		return 0;
	}
	...
	public boolean next() {
		if (isInstance) {
			...
			if (tempComplete && !LSystem.isLogo) {
				if (tmpControl != null) {
					tempComplete = false;
					setScreen(tmpControl);
				}
			}
			return false;
		}
	...
	private void setScreen(final Screen screen, boolean put) {
		synchronized (this) {
			...
			if (!running || GLEx.gl == null || tmpControl != null) {
			...
			if (screen != null) {
				if (screen instanceof LFlickerListener) {
					if (currentInput != null) {
						currentInput.setFlicker((LFlickerListener) screen);
					}
				}
			} else {
				this.isInstance = false;
