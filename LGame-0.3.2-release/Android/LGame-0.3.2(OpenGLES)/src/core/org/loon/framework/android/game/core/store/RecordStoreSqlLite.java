...
public class RecordStoreSqlLite {
	...
	private final class RecordStoreSqliteOpenHelper extends SQLiteOpenHelper {
		...
		public void onCreate(SQLiteDatabase database2) {
			...
			String a = "CREATE TABLE " + TABLENAME_RECORDSTORE + " ("
					...
					+ " INT DEFAULT 0," + "timestamp INT DEFAULT 0);";
	...
	public synchronized RecordStore getRecordStore(String recordStoreName) {
		if (recordStoreName == null) {
			throw new IllegalArgumentException(
					"The parameter 'recordStoreName' must not be null.");
		}
		Cursor resultCursor = database.query(
				RecordStoreSqlLite.TABLENAME_RECORDSTORE, null, "name = ?",
				new String[] { recordStoreName }, null, null, null);
	...
	public synchronized RecordStore getRecordStore(long pk) {
		if (pk < 0) {
			throw new IllegalArgumentException(
					"The parameter 'recordStoreName' must not have a negative value.");
		}
		Cursor result;
		RecordStore recordStore;
		result = database.query(RecordStoreSqlLite.TABLENAME_RECORDSTORE, null,
				COLUMNNAME_RECORDSTORE_RECORDSTORE_PK + " = ?",
	...
	private synchronized RecordStore extractRecordStore(Cursor result) {
		result.moveToFirst();
		int indexOfColumn;
		indexOfColumn = result
				.getColumnIndex(RecordStoreSqlLite.COLUMNNAME_RECORDSTORE_RECORDSTORE_PK);
		int id = result.getInt(indexOfColumn);

		indexOfColumn = result
				.getColumnIndex(RecordStoreSqlLite.COLUMNNAME_RECORDSTORE_VERSION);
		int version = result.getInt(indexOfColumn);

		indexOfColumn = result
				.getColumnIndex(RecordStoreSqlLite.COLUMNNAME_RECORDSTORE_NEXTID);
		int nextId = result.getInt(indexOfColumn);

		indexOfColumn = result
				.getColumnIndex(RecordStoreSqlLite.COLUMNNAME_RECORDSTORE_NAME);
		String recordStoreName = result.getString(indexOfColumn);

		indexOfColumn = result
				.getColumnIndex(RecordStoreSqlLite.COLUMNNAME_RECORDSTORE_NUMBER_OF_RECORDS);
		int numberOfRecords = result.getInt(indexOfColumn);

		indexOfColumn = result
				.getColumnIndex(RecordStoreSqlLite.COLUMNNAME_RECORDSTORE_SIZE);
	...
	public synchronized RecordStore createRecordStore(String recordStoreName)
		...
		try {
			...
			id = database.insertOrThrow(
					RecordStoreSqlLite.TABLENAME_RECORDSTORE, null, values);
			database.setTransactionSuccessful();
			database.endTransaction();
		} catch (SQLException e) {
			throw new RecordStoreException(
					"Could not insert record store row with name '"
							+ recordStoreName + "'. Reason: " + e);
		}
		if (id == -1) {
			throw new RecordStoreException(
					"Could not insert record store row with name '"
							+ recordStoreName
							+ "'. Reason: The method 'SQLiteDatabase.insertOrThrow' returned '-1' instead of throwing an exception.");
		}
		RecordStore recordStore = new RecordStore(recordStoreName, id);
		return recordStore;
	}
	...
	private void init() {
		Context context = LSystem.screenActivity;
		SQLiteOpenHelper recordStoreSqliteOpenHelper = new RecordStoreSqliteOpenHelper(
				context, "recordstoredb", null, 3);
	...
	public synchronized String[] listRecordStores() {
		Cursor resultCursor = database.query(TABLENAME_RECORDSTORE,
				new String[] { COLUMNNAME_RECORDSTORE_NAME }, null, null, null,
				null, null);
		String[] recordStores;
		try {
	...
	public synchronized void deleteRecordStore(String recordStoreName)
			throws RecordStoreNotFoundException {
		RecordStore recordStore = getRecordStore(recordStoreName);
		if (recordStore == null) {
			throw new RecordStoreNotFoundException(
					"Could not delete row in table '" + TABLENAME_RECORDSTORE
							+ "' with value '" + recordStoreName + "'");
		}
		database.beginTransaction();
		database.delete(TABLENAME_RECORDSTORE, "name = ?",
				new String[] { recordStoreName });
		database.delete(TABLENAME_RECORD, COLUMNNAME_RECORD_RECORDSTORE_FK
				+ " = ?", new String[] { Long
				.toString(recordStore.recordStorePk) });
		database.setTransactionSuccessful();
		database.endTransaction();
	}

	public synchronized int addRecord(long recordStoreFk, byte[] data)
			throws RecordStoreException {
		Cursor result = database.query(
				RecordStoreSqlLite.TABLENAME_RECORDSTORE, null,
				COLUMNNAME_RECORDSTORE_RECORDSTORE_PK + " = ?",
				new String[] { recordStoreFk + "" }, null, null, null);
		int version;
		int nextRecordId;
		int numberOfRecords;
		int size;
		...
		values
				.put(COLUMNNAME_RECORDSTORE_NUMBER_OF_RECORDS,
						newNumberOfRecords);
		values.put(COLUMNNAME_RECORDSTORE_SIZE, recordStoreSize);
		try {
			database.beginTransaction();
			database.update(TABLENAME_RECORDSTORE, values,
					COLUMNNAME_RECORDSTORE_RECORDSTORE_PK + "= ?",
					new String[] { Long.toString(recordStoreFk) });
			database.setTransactionSuccessful();
			database.endTransaction();
		} catch (Exception e) {
			throw new RecordStoreException(e.toString());
	...
	public synchronized byte[] getRecord(long recordStorePk, int recordId) {
		Cursor resultCursor = database.query(TABLENAME_RECORD,
				new String[] { COLUMNNAME_RECORD_DATA },
				COLUMNNAME_RECORD_RECORDNUMBER + "=? AND "
						+ COLUMNNAME_RECORD_RECORDSTORE_FK + "=?",
				new String[] { Long.toString(recordId),
						Long.toString(recordStorePk) }, null, null, null);
		byte[] data;
		try {
			if (resultCursor.getCount() == 0) {
				return null;
	...
	public synchronized void setRecord(long recordStorePk, int recordId,
			byte[] data) throws RecordStoreException {
		ContentValues values = new ContentValues();
		values.put(COLUMNNAME_RECORD_DATA, data);
		try {
			database.beginTransaction();
			database.update(TABLENAME_RECORD, values,
					COLUMNNAME_RECORD_RECORDSTORE_FK + "= ? AND "
	...
	public synchronized void removeRecord(long recordStorePk, int recordId)
			throws RecordStoreException {
		...
		database.update(TABLENAME_RECORDSTORE, values,
				COLUMNNAME_RECORDSTORE_RECORDSTORE_PK + "= ?",
				new String[] { Long.toString(recordStorePk) });
		database.delete(TABLENAME_RECORD, COLUMNNAME_RECORD_RECORDNUMBER
				+ "=? AND " + COLUMNNAME_RECORD_RECORDSTORE_FK + "=?",
				new String[] { Integer.toString(recordId),
						Long.toString(recordStorePk) });
	}

	public synchronized int[] getRecordIdsForRecordStore(long recordStorePk) {
		Cursor resultCursor = database.query(TABLENAME_RECORD,
				new String[] { COLUMNNAME_RECORD_RECORDNUMBER },
				COLUMNNAME_RECORD_RECORDSTORE_FK + "=?", new String[] { Long
						.toString(recordStorePk) }, null, null,
				COLUMNNAME_RECORD_RECORDNUMBER + " ASC");
		int[] result;
		try {
