...
import org.loon.framework.android.game.stg.STGHero.HeroTouch;
...
public abstract class STGScreen extends Screen {
	...
	boolean setPlaneAnime(int index, boolean anime) {
		...
		if (plane == null) {
			...
			if (plane.animation = anime) {
				...
				Enumeration<Integer> enumeration = plane.images.keys();
				for (int j = 0; enumeration.hasMoreElements(); ++j) {
					plane.animeList[j] = enumeration.nextElement();
				}
	...
	boolean nextCommand() {
		if (this.sequenceCount < 0) {
			Enumeration<STGObject> e = this.stgObjects.elements();
