...
public class LLKScreen extends Screen {
	...
	public void onLoad() {
		...
		images[8] = LPKResource.openLImage(res, "a0.png").getTexture();
		images[9] = LPKResource.openLImage(res, "dot.png").getTexture();
		images[10] = LPKResource.openLImage(res, "background.jpg").getTexture();
		images[11] = LPKResource.openLImage(res, "role0.png").getTexture();
		images[12] = LPKResource.openLImage(res, "role1.png").getTexture();
		images[13] = LPKResource.openLImage(res, "role2.png").getTexture();
		images[14] = LPKResource.openLImage(res, "win.png").getTexture();
		images[15] = LPKResource.openLImage(res, "start.png").getTexture();
		images[16] = LPKResource.openLImage(res, "gameover.png").getTexture();
		...
		mes = new LMessage(getImage(14), (getWidth() - 460) / 2,
			...
			public void doClick() {
				...
				if (HELP_MES.equalsIgnoreCase(getMessage()) && isComplete()) {
					setVisible(false);
					select = new LSelect(getImage(14), (LLKScreen.this
							.getWidth() - 460) / 2,
					...
					select.setFontColor(GLColor.black);
	...
	private LTimer waitTimer = new LTimer(LSystem.SECOND);

	public void alter(LTimerContext t) {
		if (!isOnLoadComplete()) {
			if (waitTimer.action(t)) {
				wait.next();
			}
		}
		if (isWait()) {
