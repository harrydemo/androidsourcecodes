...
public class Main extends Screen {
	...
	class Bullet extends Actor {
		...
		public void action(long t) {
			...
			for (int i = 0; i < 6; i++) {
				// 矫正弹道位置
				double angle = MathUtils.toRadians(this.dir);
	...
	class Turret extends Actor {
		...
		public void action(long t) {
			...
			if (!es.isEmpty()) {
				...
				// 旋转炮台对准Enemy坐标
				setRotation((int) MathUtils.toDegrees(MathUtils.atan2((target
						.getY() - this.getY()), (target.getX() - this.getX()))));
			...
			if (this.delay > 0) {
				...
				// 计算炮击点
				int x = (int) MathUtils.round(MathUtils.cos(MathUtils
						.toRadians(this.getRotation()))
						* (float) bullet.getWidth() * 2)
						+ this.x();

				int y = (int) MathUtils.round(MathUtils.sin(MathUtils
						.toRadians(this.getRotation()))
						* (float) bullet.getHeight() * 2)
						+ this.y();
				// 注入炮弹到Layer
	...
	public static void main(String[] args) {
		GameScene frame = new GameScene("TD_OpenGL_TEST", 480, 320);
		frame.setShowFPS(true);
		frame.setShowMemory(false);
		frame.setScreen(new Main());
		frame.showScreen();
	}
}
