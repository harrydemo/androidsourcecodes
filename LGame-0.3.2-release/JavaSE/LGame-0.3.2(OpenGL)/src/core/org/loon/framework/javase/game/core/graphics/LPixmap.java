...
import org.loon.framework.javase.game.core.geom.Polygon.Polygon2i;
 ...
 * @version 0.1
 */
public final class LPixmap extends LGraphicsMath implements LRelease {
	...
	public LPixmap(int w, int h, boolean hasAlpha, boolean natural) {
		this.hasAlpha = hasAlpha;
		this.width = w;
		this.height = h;
		this.texWidth = GLEx.toPowerOfTwo(w);
		this.texHeight = GLEx.toPowerOfTwo(h);
		this.makeDrawPixels(width, height, hasAlpha);
	}

	public LPixmap(String fileName, boolean natural) {
		this.pixelToImage = GraphicsUtils.loadBufferedImage(fileName);
		this.width = pixelToImage.getWidth();
		this.height = pixelToImage.getHeight();
		this.hasAlpha = pixelToImage.getColorModel().hasAlpha();
		if (hasAlpha) {
			this.transparent = 0;
		} else {
			this.transparent = pixelToImage.getTransparency();
		}
		this.texWidth = GLEx.toPowerOfTwo(width);
		this.texHeight = GLEx.toPowerOfTwo(height);
		if (hasAlpha) {
			this.transparent = 0;
		} else {
			this.transparent = pixelToImage.getTransparency();
		}
		boolean useInteger = (pixelToImage.getRaster().getTransferType() == DataBuffer.TYPE_INT);
		if (!useInteger) {
			BufferedImage tmp = new BufferedImage(width, height,
					hasAlpha ? BufferedImage.TYPE_INT_ARGB
							: BufferedImage.TYPE_INT_RGB);
			Graphics2D g = tmp.createGraphics();
			g.drawImage(pixelToImage, x, y, null);
			g.dispose();
			BufferedImage temp = pixelToImage;
			pixelToImage = tmp;
			if (temp != null) {
				temp.flush();
				temp = null;
		...
		this.drawPixels = DataBufferHelper.getDataInt(dataBuffer);
	...
	public LPixmap(String fileName) {
		try {
			...
			if (GLEx.isPowerOfTwo(width) && GLEx.isPowerOfTwo(height)) {
				...
				g.drawImage(pixelToImage, 0, 0, null);

				if (this.height < texHeight - 1) {
					GLLoader.copyArea(texImage, g, 0, 0, width, 1, 0,
							texHeight - 1);
					GLLoader.copyArea(texImage, g, 0, height - 1, width, 1, 0,
							1);
				}
				if (this.width < texWidth - 1) {
					GLLoader.copyArea(texImage, g, 0, 0, 1, height,
							texWidth - 1, 0);
					GLLoader.copyArea(texImage, g, width - 1, 0, 1, height, 1,
							0);
				}
				BufferedImage tmp = pixelToImage;
				pixelToImage = texImage;
				if (tmp != null) {
					tmp.flush();
					tmp = null;
				}
	...
	private void makeDrawPixels(int w, int h, boolean hasAlpha) {
		if (pixelToImage == null) {
			this.pixelToImage = new BufferedImage(w, h,
					hasAlpha ? BufferedImage.TYPE_INT_ARGB
							: BufferedImage.TYPE_INT_RGB);
		}
		if (hasAlpha) {
	...
	public void fillRect(int x, int y, int width, int height) {
		...
		float maxX = Math.min(x + width - 1 + translateX, clip.x + clip.width
				- 1);
		float maxY = Math.min(y + height - 1 + translateY, clip.y + clip.height
				- 1);
		for (float row = Math.max(y + translateY, clip.y), rowOffset = row
				* width; row <= maxY; row++, rowOffset += width) {
			for (float col = Math.max(x + translateX, clip.x); col <= maxX; col++) {
				drawPoint(drawPixels, (int) (col + rowOffset));
			}
		}
	...
	private void drawLineImpl(int x1, int x2, int y) {
		...
		if (y >= clip.y && y < clip.y + clip.height) {
			y *= width;
			float maxX = Math.min(x2, clip.x + clip.width - 1);
			if (drawPixels != null) {
				for (float x = Math.max(x1, clip.x); x <= maxX; x++) {
					drawPoint(drawPixels, (int) (x + y));
				}
			}
		}
	}
	...
	private void drawVerticalLine(int x, int y1, int y2) {
		if (x >= clip.x && x < clip.x + clip.width) {
			float maxY = Math.min(y2, clip.y + clip.height - 1) * width;
			if (drawPixels != null) {
				for (float y = Math.max(y1, clip.y) * width; y <= maxY; y += width) {
					drawPoint(drawPixels, (int) (x + y));
				}
			}
		}
	...
	private void drawArcImpl(int xPoints[], int yPoints[], int nPoints,
		...
		if (y >= clip.y && y < clip.y + clip.height) {
			for (float x = Math.max(xLeft, clip.x); x <= xRight; x++) {
	...
	public byte[] getBytePixles() {
		return hasAlpha ? GLColor.argbToRGBA(drawPixels) : GLColor
				.argbToRGB(drawPixels);
	}
