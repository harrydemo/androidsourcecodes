...
public class BSPCollisionChecker implements CollisionChecker {
	...
	public void addObject(Actor actor) {
		...
		if (this.bspTree == null) {
			...
			if (bounds.getWidth() > bounds.getHeight()) {
				treeArea = 0;
				by = (int) bounds.getMiddleX();
			} else {
				treeArea = 1;
				by = (int) bounds.getMiddleY();
			}
			...
			while (!treeArea1.contains(bounds)) {
				...
				if (bounds.getX() < treeArea1.getX()) {
					by = (int) (treeArea1.getX() - treeArea1.getWidth());
					...
					newTop.setSplitPos((int) treeArea1.getX());
				...
				if (bounds.getY() < treeArea1.getY()) {
					by = (int) (treeArea1.getY() - treeArea1.getHeight());
					newArea = new RectBox(treeArea1.getX(), by, treeArea1
							.getWidth(), treeArea1.getBottom() - by);
					newTop = getBSPNode();
					newTop.getArea().copy(newArea);
					newTop.setSplitAxis(1);
					newTop.setSplitPos((int) treeArea1.getY());
					newTop.setChild(1, this.bspTree);
					this.bspTree = newTop;
					treeArea1 = newArea;
				}
				...
				if (bounds.getBottom() > treeArea1.getBottom()) {
					by = (int) (treeArea1.getBottom() + treeArea1.getHeight());
					newArea = new RectBox(treeArea1.getX(), treeArea1.getY(),
							treeArea1.getWidth(), by - treeArea1.getY());
					newTop = getBSPNode();
					newTop.getArea().copy(newArea);
					newTop.setSplitAxis(1);
					newTop.setSplitPos((int) treeArea1.getBottom());
					newTop.setChild(0, this.bspTree);
					this.bspTree = newTop;
					treeArea1 = newArea;
				}
	...
	private BSPCollisionNode createNewNode(RectBox area) {
		...
		if (area.getWidth() > area.getHeight()) {
			splitAxis = 0;
			splitPos = (int) area.getMiddleX();
		} else {
			splitAxis = 1;
			splitPos = (int) area.getMiddleY();
		}
