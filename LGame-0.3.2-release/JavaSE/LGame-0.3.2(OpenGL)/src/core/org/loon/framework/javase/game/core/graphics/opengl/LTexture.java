...
import org.loon.framework.javase.game.action.collision.CollisionMask;
import org.loon.framework.javase.game.action.sprite.Mask;
...
public class LTexture implements LRelease {
	...
	boolean replace, string, hasAlpha;
	...
	public void draw(BufferedImage bit, int x, int y) {
		...
		if (GLEx.gl != null) {
			...
			LTextureData data = GLLoader.getTextureData(bit);
			int srcPixelFormat = data.hasAlpha ? GL.GL_RGBA : GL.GL_RGB;

			while (height >= 1 || width >= 1 && level < 4) {
				GLEx.gl10.glTexSubImage2D(GL11.GL_TEXTURE_2D, level, x, y,
						width, height, srcPixelFormat, GL11.GL_UNSIGNED_BYTE,
						data.getSource());
				if (height == 1 || width == 1) {
					break;
				}
				...
				BufferedImage bitmap2 = GraphicsUtils.getResize(bit, width,
						height);
				bit.flush();
				bit = null;
				bit = bitmap2;
	...
	public void draw(LPixmap pixmap, int x, int y) {
		...
		if (GLEx.gl != null) {
			...
			LTextureData textureData = GLLoader.getTextureData(pixmap);

			Buffer buffer = textureData.getSource();

			int srcPixelFormat = textureData.hasAlpha ? GL.GL_RGBA : GL.GL_RGB;

			GLEx.gl.glTexSubImage2D(GL.GL_TEXTURE_2D, 0, x, y, width, height,
					srcPixelFormat, GL.GL_UNSIGNED_BYTE, buffer);
	...
	public void setFormat(Format format) {
		...
		gl10.glTexParameteri(GL.GL_TEXTURE_2D, GL.GL_TEXTURE_MIN_FILTER,
				minFilter);
		gl10.glTexParameteri(GL.GL_TEXTURE_2D, GL.GL_TEXTURE_MAG_FILTER,
				maxFilter);
		if (GLContext.getCapabilities().GL_EXT_texture_mirror_clamp) {
			gl10.glTexParameteri(GL.GL_TEXTURE_2D, GL.GL_TEXTURE_WRAP_S,
					EXTTextureMirrorClamp.GL_MIRROR_CLAMP_TO_EDGE_EXT);
			gl10.glTexParameteri(GL.GL_TEXTURE_2D, GL.GL_TEXTURE_WRAP_T,
					EXTTextureMirrorClamp.GL_MIRROR_CLAMP_TO_EDGE_EXT);
		} else {
			gl10.glTexParameteri(GL.GL_TEXTURE_2D, GL.GL_TEXTURE_WRAP_S, wrapS);
			gl10.glTexParameteri(GL.GL_TEXTURE_2D, GL.GL_TEXTURE_WRAP_T, wrapT);
		}
