...
import org.loon.framework.javase.game.physics.PolygonDef;
...
public class Body {
	...
	public Fixture createFixture(FixtureDef def) {
		Fixture fixture = new Fixture(world, this, jniCreateFixture(addr,
	...
	public Fixture createShape(PolygonDef def) {
		PolygonShape polyShape = new PolygonShape();
		polyShape.set(def.getVertexs());
		def.shape = polyShape;
		return createFixture(def);
	}
	
	private native long jniCreateFixture(long addr, long shapeAddr,
			float friction, float restitution, float density, boolean isSensor,
			short filterCategoryBits, short filterMaskBits,
	 ...
	 */
	private final Vector2f worldCenter = new Vector2f();
	 ...
	 */
	private final Vector2f localCenter = new Vector2f();
	...
	private native void jniGetLinearVelocity(long addr,
			float[] tmpLinearVelocity);
	...
	private native void jniApplyLinearImpulse(long addr, float impulseX,
			float impulseY, float pointX, float pointY);
	 ...
	 */
	private final Vector2f localPoint = new Vector2f();

	public Vector2f getWorldPoint(Vector2f localPoint) {
	 ...
	 */
	private final Vector2f worldVector = new Vector2f();
	 ...
	 */
	public final Vector2f localPoint2 = new Vector2f();
	 ...
	 */
	public final Vector2f localVector = new Vector2f();
	 ...
	 */
	public final Vector2f linVelWorld = new Vector2f();

	public Vector2f getLinearVelocityFromWorldPoint(Vector2f worldPoint) {
		jniGetLinearVelocityFromWorldPoint(addr, worldPoint.x, worldPoint.y,
				tmp);
		linVelWorld.x = tmp[0];
		linVelWorld.y = tmp[1];
		return linVelWorld;
	...
	private native void jniGetLinearVelocityFromWorldPoint(long addr,
			float worldPointX, float worldPointY, float[] linVelWorld);
	 ...
	 * @return the world velocity of a point.
	 */
	public final Vector2f linVelLoc = new Vector2f();

	public Vector2f getLinearVelocityFromLocalPoint(Vector2f localPoint) {
		jniGetLinearVelocityFromLocalPoint(addr, localPoint.x, localPoint.y,
				tmp);
		linVelLoc.x = tmp[0];
		linVelLoc.y = tmp[1];
		return linVelLoc;
	...
	private native void jniGetLinearVelocityFromLocalPoint(long addr,
			float localPointX, float localPointY, float[] linVelLoc);
