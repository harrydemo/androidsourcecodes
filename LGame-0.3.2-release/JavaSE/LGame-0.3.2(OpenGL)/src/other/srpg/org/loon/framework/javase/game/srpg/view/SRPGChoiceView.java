...
public class SRPGChoiceView extends SRPGView {
	...
	public void set(LTexture image, String[][] messages, int[] joint,
		...
		LSTRTexture.bindStringLazy(font, sbr.toString());
		...
		// 颜色集合（用以改变指定行选项颜色）
		this.colors = new GLColor[messages.length];
	...
	public synchronized void drawChoice(GLEx g) {
		...
		try {
			...
			if (i >= 0 && i < maxSize) {
				GLGradation.getInstance(GLColor.white, GLColor.black,
	...
	public void setColor(GLColor acolor[]) {
		colors = acolor;
	}

	public void setColor(int i, GLColor color1) {
		colors[i] = color1;
	}

	public GLColor getColor(int i) {
		return colors[i];
	}
