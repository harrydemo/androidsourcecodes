...
import org.loon.framework.javase.game.action.BallTo;
import org.loon.framework.javase.game.action.CircleTo;
import org.loon.framework.javase.game.action.FadeTo;
import org.loon.framework.javase.game.action.FireTo;
import org.loon.framework.javase.game.action.JumpTo;
import org.loon.framework.javase.game.action.MoveTo;
import org.loon.framework.javase.game.action.RotateTo;
import org.loon.framework.javase.game.action.ScaleTo;
...
public class Actor implements ImageFilterType, LRelease {
	...
	public final static int FADE_SPEED = 120;

	public final static float ROTATE_SPEED = 2.0F;

	public final static float JUMP_GRAVITY = 0.3F;
	...
	public Actor(Animation animation, int x, int y) {
		...
		this.x = x;
		this.y = y;
		this.setImage(animation.getSpriteImage().getLImage());
	}
	...
	public FadeTo fadeOut() {
		failIfNotInLayer();
		return gameLayer.callFadeOutTo(this, FADE_SPEED);
	}
	...
	public FadeTo fadeIn() {
		failIfNotInLayer();
		return gameLayer.callFadeInTo(this, FADE_SPEED);
	}
	...
	public RotateTo rotateTo(float rotate) {
		return rotateTo(rotate, ROTATE_SPEED);
	}
	...
	public JumpTo jumpTo(int jump) {
		return jumpTo(jump, JUMP_GRAVITY);
	}
	...
	void update(long elapsedTime) {
		if (timer.action(elapsedTime)) {
			if (isAnimation) {
				...
				setImage(animation.getSpriteImage().getLImage());
			}
			action(elapsedTime);
		}
	}
	...
	private void calcBounds() {
		...
		if (layer != null) {
			...
			if (this.image == null) {
				...
				this.boundingRect = NumberUtils.getBounds(x, y, this.image
						.getWidth(), this.image.getHeight(), rotation,
						boundingRect);
			}
		}
	}
	...
	public List getCollisionObjects(int dx, int dy, Class cls) {
		this.failIfNotInLayer();
		return this.gameLayer.getCollisionObjectsAt(this.x + dx, this.y + dy,
				cls);
	}

	public Actor getOnlyCollisionObject(int dx, int dy, Class cls) {
		this.failIfNotInLayer();
		return this.gameLayer.getOnlyObjectAt(this, this.x + dx, this.y + dy,
				cls);
	}

	public List getCollisionObjects(int radius, Class cls) {
		this.failIfNotInLayer();
		List inRange = this.gameLayer.getObjectsInRange(this.x, this.y, radius,
				cls);
		inRange.remove(this);
		return inRange;
	}
	...
	public Actor getOnlyCollisionObject(Class cls) {
		this.failIfNotInLayer();
