...
public class PolygonSprite implements ISprite {
	...
	public PolygonSprite(Image image, int x, int y) {
		this.visible = true;
		this.showImage = true;
		...
		this.imageBuffer = spriteImage.getImage();
	...
	public void rotate(final int angle) {
		...
		BufferedImage tmp = sr.getBitmap(0);
		if (showImage) {
			imageBuffer = tmp;
		}
		pixels = GraphicsUtils.getPixels(imageBuffer);
		finalPolygon = spriteImage.makePolygon(pixels, 0, 0, 0, 0, width,
				height);
		makePolygon();
	}
	...
	public void createUI(LGraphics g) {
		if (visible) {
			if (alpha > 0.1 && alpha < 1.0) {
				g.setAlpha(alpha);
				if (showImage) {
					g.drawImage(imageBuffer, location.x(), location.y());
				}
				if (showPolygon) {
