...
import org.loon.framework.javase.game.core.EmulatorButton;
...
import org.loon.framework.javase.game.srpg.ability.SRPGAbilityOption;
import org.loon.framework.javase.game.srpg.ability.SRPGDamageAverage;
...
import org.loon.framework.javase.game.srpg.actor.SRPGPosition;
import org.loon.framework.javase.game.srpg.actor.SRPGStatus;
import org.loon.framework.javase.game.srpg.effect.SRPGEffect;
import org.loon.framework.javase.game.srpg.effect.SRPGEffectFactory;
import org.loon.framework.javase.game.srpg.effect.SRPGNumberEffect;
import org.loon.framework.javase.game.srpg.effect.SRPGPhaseEffect;
import org.loon.framework.javase.game.srpg.effect.SRPGUpperEffect;
...
import org.loon.framework.javase.game.srpg.field.SRPGFieldMove;
import org.loon.framework.javase.game.srpg.field.SRPGField;
import org.loon.framework.javase.game.srpg.field.SRPGTeams;
import org.loon.framework.javase.game.srpg.view.SRPGMessageListener;
import org.loon.framework.javase.game.srpg.view.SRPGMessageView;
import org.loon.framework.javase.game.srpg.view.SRPGAbilityNameView;
import org.loon.framework.javase.game.srpg.view.SRPGActorStatusView;
import org.loon.framework.javase.game.srpg.view.SRPGChoiceView;
import org.loon.framework.javase.game.srpg.view.SRPGDamageExpectView;
import org.loon.framework.javase.game.srpg.view.SRPGFieldChoiceView;
import org.loon.framework.javase.game.srpg.view.SRPGMiniStatusView;
import org.loon.framework.javase.game.srpg.view.SRPGDrawView;
import org.loon.framework.javase.game.srpg.view.SRPGAvgView;
import org.loon.framework.javase.game.utils.GraphicsUtils;
import org.loon.framework.javase.game.utils.RecordStoreUtils;
...
public abstract class SRPGScreen extends Screen implements SRPGType, Runnable {
	...
	private Image cursor, messageImage;
	...
	private final static Color[] colors = { new Color(255, 0, 32),
			new Color(0, 192, 0), new Color(192, 192, 0) };

	private Color moving_our = new Color(0, 128, 255),
			moving_other = new Color(255, 100, 20), attack_target = new Color(
					255, 192, 0), attack_range = new Color(255, 128, 64),
			moving_change = new Color(0, 64, 255), hero_flag = Color.blue,
	...
	public SRPGScreen(String fileName, Image img) {
	...
	public SRPGScreen(String fileName, Image img, int row, int col) {
		this(null, fileName, img, row, col);
	}

	public SRPGScreen(SRPGFieldElements elements, String fileName, Image img,
			int row, int col) {
	 ...
	 * @param g
	 */
	protected void initLoading(LGraphics g) {
	 ...
	 * @param g
	 */
	public void background(LGraphics g) {
	 ...
	 * @param g
	 */
	public void foreground(LGraphics g) {
	}
	...
	protected void makeEmulatorButton(String f, int x, int y) {
		Image img = GraphicsUtils.loadNotCacheImage(f);
		makeEmulatorButtons(img, null, x, y);
	}
	 ...
	 * @param y
	 */
	protected void makeEmulatorButtons(Image on, Image un, int x, int y) {
	 ...
	 * @return
	 */
	private Image createTempImage() {
		if (messageImage == null) {
			messageImage = GraphicsUtils.createImage(getWidth() - 40,
					getHeight() / 2 - 20, true);
			Graphics g = messageImage.getGraphics();
			GraphicsUtils.setAlpha(g, 0.5D);
			GraphicsUtils.rectFill(g, 0, 0, messageImage.getWidth(null),
					messageImage.getHeight(null), Color.black);
	...
	protected boolean beforeDamageValue(int number, int index,
			SRPGAbilityOption abilityoption, boolean flag) {
	...
	protected void setDamageValueImplement(int abilityIndex, int roleIndex,
			SRPGAbilityOption abilityoption, boolean flag) {
	 ...
	 * @param i
	 */
	public synchronized void callActorMove(int i) {
	 ...
	 * @param res
	 */
	public synchronized void callActorMove(int i, int[][] res) {
		setCenterActor(i);
		SRPGActor actor = srpgActors.find(i);
		...
		for (int j = 0; j < srpgActors.size(); j++) {
			...
			if (status.exp != -100) {
				setEffect(new SRPGNumberEffect(actor.getPosX() * tileWidth,
						actor.getPosY() * tileHeight, Color.red, "Level Up!"));
				...
				setEffect(new SRPGNumberEffect(actor.getPosX() * tileWidth,
						actor.getPosY() * tileHeight, Color.red, "Level Down!"));
			}
	...
	private final void drawGrid(LGraphics g) {
		for (int x = 0; x < srpgField.getWidth() + 1; x++) {
			g.drawLine(x * tileWidth - cam_x, 0 - cam_y, x * tileWidth - cam_x,
					srpgField.getHeight() * tileWidth - cam_y);
		}
		for (int y = 0; y < srpgField.getHeight() + 1; y++) {
			g.drawLine(0 - cam_x, y * tileHeight - cam_y, srpgField.getWidth()
					* tileWidth - cam_x, y * tileHeight - cam_y);
		}
	}
	...
	public synchronized void draw(LGraphics g) {
					...
					if (procFlag != PROC_TARGETSURE && procFlag != PROC_COUNTER) {
						...
						if (getTargetTrue(SRPGAbilityFactory
								...
								srpgPosition.number, srpgActors
										.checkActor(x, y))) {
							g.setColor(attack_target);
						} else {
							g.setColor(attack_range);
						}
					}
					drawCube(g, g.getColor(), x, y);
				}
			}
			if (isGridAlpha) {
				g.setAlpha(1.0F);
		...
		if (srpgPosition.counter && procFlag == PROC_COUNTER) {
			...
			drawCube(g, attack_target, srpgActors.find(srpgPosition.enemy)
		...
		if (procFlag == PROC_CHANGEVECTOR) {
			...
			for (int d = 0; d < 4; d++) {
				...
				if (tx >= 0 && ty >= 0 && tx < srpgField.getWidth()
					...
					drawCube(g, moving_change, tx, ty);
				}
			}
			if (isGridAlpha) {
				g.setAlpha(1.0F);
		...
		for (int i = 0; i < srpgActors.size(); i++) {
			...
			if (isCursor) {
				...
				if (index == i) {
					g.drawImage(cursor, actor.drawX() - cam_x,
							(actor.drawY() - cam_y));
				}
			}
		...
		for (int i = 0; i < srpgActors.size(); i++) {
			...
			if (result != null) {
				...
				g.drawStyleString(result, (actor.drawX() - size) - cam_x, actor
						.drawY()
						- 3 - cam_y, Color.black, Color.white);
			}
		}
	 ...
	 * @param y
	 */
	private final void drawCube(LGraphics g, Color color, int x, int y) {
		...
		try {
			...
			if (srpgHelper == null) {
				srpgHelper = new SRPGMessageView(message, language_index,
						DEFAULT_FONT,
						(getWidth() - messageImage.getWidth(null)) / 2,
						getHeight() - messageImage.getHeight(null) - 10,
						messageImage.getWidth(null), messageImage
								.getHeight(null), offsetLeft, offsetTop);
			} else {
				srpgHelper.dispose();
				srpgHelper.set(message, language_index, DEFAULT_FONT,
						...
						getHeight() - messageImage.getHeight(null) - 10,
						messageImage.getWidth(null), messageImage
								.getHeight(null), offsetLeft, offsetTop, true);
			}
			...
			if (index != -1) {
				SRPGActor actor = srpgActors.find(index);
				LImage img = actor.getActorStatus().face;
				if (img != null) {
					srpgHelper.setFaceImage(img.getBufferedImage());
				}
			}
			if (flag) {
	...
	public final void setMessageCharacter(Image img, int x, int y) {
	...
	public final void setHelper(String[] message, int x, int y, int w, int h,
		...
		srpgHelper.setBackground((Image) null);
		if (flag) {
			srpgHelper.printWait(this);
		} else {
			srpgHelper.messageWait(this);
	...
	public final void setHelperCharacter(Image img, int x, int y) {
		if (srpgHelper != null) {
	...
	public final synchronized void onTouchDown(final LTouch e) {
	...
	public final synchronized void onTouchUp(LTouch e) {
		if (!isEventLoop || isSrpgTouchLock) {
			return;
		}
		onUp(e);
	...
	public final synchronized void onTouchMove(LTouch e) {
		if (!isEventLoop || isSrpgTouchLock || isSrpgNoMove) {
			return;
		}
		if ((!srpgDrawView.isExist() || !srpgDrawView.isExist()
				|| !srpgHelper.messageExist() || !srpgAvgView.isExist() || !srpgHelper
				.isLock())) {
			if (srpgChoiceView.isExist()) {
	...
	private synchronized final void callSRPGBattleProcEvent(
		...
		case PROC_NORMAL:
			// 确定
			if (type == SRPGEvent.EVENT_SUBMIT && isTouchClick()) {
				// 角色存在
				int index = srpgActors.checkActor(x, y);
				if (index != -1) {
					SRPGActor actor = srpgActors.find(index);
	...
	private synchronized final void callSRPGBattleEvent(SRPGEvent e) {
		...
		if (e.type == SRPGEvent.EVENT_SUBMIT && isTouchClick()) {
	...
	public void setHeroMoveColor(Color c) {
		this.moving_our = c;
	}

	public void setEnemyMoveColor(Color c) {
		this.moving_other = c;
	}

	public void setAttackTargetColor(Color c) {
		this.attack_target = c;
	}

	public void setAttackRangeColor(Color c) {
		this.attack_range = c;
	}

	public void setVectorChangeColor(Color c) {
	...
	public void setMessageImage(Image img) {
		this.messageImage = img;
	}

	public void setMessageImage(String fileName) {
		setMessageImage(GraphicsUtils.loadImage(fileName));
	}
	...
	public final LMessage getAvgMessage() {
		if (srpgAvgView != null) {
