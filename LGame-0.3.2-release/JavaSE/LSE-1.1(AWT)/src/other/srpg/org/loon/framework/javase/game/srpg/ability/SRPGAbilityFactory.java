...
public class SRPGAbilityFactory {
	...
	final static ArrayList<SRPGAbility> lazyAbilityClass = new ArrayList<SRPGAbility>(24);
	...
	public SRPGDamageData getDamageExpect(SRPGField field, SRPGActors actors,
		...
		this.atk = field.getPosMapElement(attacker.getPosX(), attacker
				.getPosY()).atk;
		this.def = field.getPosMapElement(defender.getPosX(), defender
				.getPosY()).def;
			...
			if ((ability.getMP(status) <= status.mp || status.mp == -1)
					...
					&& (flag ? ability.getCounter() != 2
							: ability.getCounter() != 1
									&& ability.getTarget() != 1)) {
				result = (int[]) CollectionUtils.expand(result,
						1);
	...
	public static int[][] setAttackRange(int[] range, SRPGField field, int x,
			int y) {
		SRPGFieldMove fieldMove = SRPGFieldMove.getInstance(field.getMoveSpace(19));
