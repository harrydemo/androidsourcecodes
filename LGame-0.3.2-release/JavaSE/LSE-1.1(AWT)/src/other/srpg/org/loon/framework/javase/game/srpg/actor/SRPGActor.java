...
public class SRPGActor implements LRelease {
	...
	public SRPGActor(SRPGStatus status, AnimationHelper animation, int tileWidth,
			int tileHeight) {
		this(status, animation, null, tileWidth, tileHeight);
	}

	public SRPGActor(SRPGStatus status, AnimationHelper mv, AnimationHelper atk,
	 ...
	 * @return
	 */
	public Image[] getAnimationImages(final AnimationHelper o, final int d) {
		Image[] result = null;
		switch (d) {
		// 向下移动
	...
	public Image getImage() {
		Image[] img = getAnimationImages(move, SRPGType.MOVE_DOWN);
		if (img != null) {
	 ...
	 * @param y
	 */
	private void drawActor(LGraphics g, int x, int y) {
		...
		Image img = actionImage[actionCount];
		// 移动中(此处可能会修改加入特殊效果)
		if (isMoving) {
			g.drawImage(img, x + (tileWidth - img.getWidth(null)) / 2, y
					+ (tileHeight - img.getHeight(null)) / 2);
			// 角色行动完毕
		} else if (isEnd) {
			g.drawImage(endImage, x + (tileWidth - img.getWidth(null)) / 2, y
					+ (tileHeight - img.getHeight(null)) / 2);
			// 其它情况
		} else {
			g.drawImage(img, x + (tileWidth - img.getWidth(null)) / 2, y
	 ...
	 * @param y
	 */
	public void draw(final LGraphics g, int x, int y) {
		if (isVisible()) {
	...
	public void setActionEnd(boolean e) {
		if (e) {
			if (endImage == null) {
				endImage = ImageFilterFactory.getGray(actionImage[actionCount]);
			}
		} else {
			if (endImage != null) {
				endImage.flush();
