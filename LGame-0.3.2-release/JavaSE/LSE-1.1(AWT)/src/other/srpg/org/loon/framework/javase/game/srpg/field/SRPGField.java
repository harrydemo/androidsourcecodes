...
public class SRPGField implements LRelease {
	...
	private BufferedImage bigImageMap, tempFieldImage;
		...
		case SRPGType.FIELD_NORMAL:
			...
			for (int ny = 0; ny < maxY; ny++) {
				for (int nx = 0; nx < maxX; nx++) {
					...
					g.drawImage(element.image, nx * tileWidth - x2, ny
							* tileHeight - y2);
				}
			}
			break;
	...
	public Image getBigImageMap() {
		return bigImageMap;
	}

	public void setBigImageMap(String fileName) {
		setBigImageMap(GraphicsUtils.loadBufferedImage(fileName));
	}

	public void setBigImageMap(BufferedImage img) {
		if (img != null) {
			if (img.getWidth() == drawWidth && img.getHeight() == drawHeight) {
				this.bigImageMap = img;
				if (bigImageMap.getType() != BufferedImage.TYPE_INT_RGB) {
					BufferedImage temp = bigImageMap;
					BufferedImage newBitmap = GraphicsUtils.copy(temp,
