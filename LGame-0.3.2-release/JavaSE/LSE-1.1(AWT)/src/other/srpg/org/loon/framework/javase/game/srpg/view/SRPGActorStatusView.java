...
public class SRPGActorStatusView extends SRPGDrawView {
	...
	private final static Color colorAbility = new Color(220, 220, 220),
			color0 = new Color(255, 128, 96),
			color1 = new Color(192, 220, 255),
			color2 = new Color(192, 192, 192),
			color3 = new Color(255, 192, 192);
	...
	public void draw(LGraphics g) {
		...
		LGradation.getInstance(color, Color.black, super.width, super.height)
		...
		for (int i = 9; i < SRPGStatus.STATUS_MAX; i++) {
			...
			g.drawStyleString(mes, 260 + size + super.left, (75 - index * 12)
					+ super.top, Color.black, Color.white);
			index++;
		}
		...
		for (index = 0; index < cstatus.length; index++) {
			if (cstatus[index][0] != null) {
				...
				g.drawString(cstatus[index][0], 5 + super.left, 135 + index
						* 18 + super.top);
				g.drawString(cstatus[index][1], 130 + super.left, 135 + index
						* 18 + super.top);
				g.drawString(cstatus[index][2], 165 + super.left, 135 + index
						* 18 + super.top);
				g.drawString(cstatus[index][3], 190 + super.left, 135 + index
						* 18 + super.top);
			}
		}
		if (s.face != null) {
