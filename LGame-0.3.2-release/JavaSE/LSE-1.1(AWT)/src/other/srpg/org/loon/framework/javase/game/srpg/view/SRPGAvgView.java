...
import org.loon.framework.javase.game.action.avg.AVGCG;
import org.loon.framework.javase.game.action.avg.AVGChara;
import org.loon.framework.javase.game.action.avg.command.Command;
import org.loon.framework.javase.game.action.avg.command.CommandType;
...
public class SRPGAvgView extends SRPGView implements LRelease {
	...
	public SRPGAvgView(final SRPGScreen screen, final Image img, final int w,
			final int h) {
		this.screen = screen;
		this.width = w;
		this.height = h;
	...
	public void setMessageImage(Image img, boolean flag) {
		if (img != null) {
			if (message == null) {
				...
				for (; isRunning = command.next();) {
					...
					if (cmdFlag.equalsIgnoreCase(CommandType.L_FLASH)) {
						...
						if (color == null && colors != null
								&& colors.length == 3) {
							color = new Color(Integer.valueOf(colors[0])
					...
					if (cmdFlag.equalsIgnoreCase(CommandType.L_CG)) {
						...
						if (mesFlag.equalsIgnoreCase(CommandType.L_DEL)) {
							...
							scrCG.addImage(mesFlag, x, y, height);
						}
						continue;
					}
					if (screen != null) {
