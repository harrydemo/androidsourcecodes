...
public class SRPGMessageView extends SRPGView implements LRelease {
	...
	private Image face, background, character;
				...
				if (isLeft) {
					...
					if (!isEnglish) {
						if (languageIndex == 0 || languageIndex == 1) {
							...
							message[i].setMessageLength(font
									.stringWidth(mes[i])
									/ mes[i].length() - 1);
	...
	public void setBackground(LImage img) {
		setBackground(img.getBufferedImage());
	}
	...
	public void setCharacterImage(Image img, int x, int y) {
		this.character = img;
	...
	public synchronized void draw(LGraphics g) {
		if (isExist()) {
			...
			Font font = g.getFont();
			if (face != null) {
				int w = face.getWidth(null);
				int h = face.getHeight(null);
				int x = mes.getX();
				int y = mes.getY() - h;
			...
			if (background == null) {
				LGradation.getInstance(Color.white, Color.black,
