...
public class QueryPhone extends BaseActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setView(R.layout.query_phone);
		SetTitle(getString(R.string.number_mobile_info));
	...
	private void setListener() {
		query_btn.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				boolean hasNet = new InternetHelper(QueryPhone.this)
				...
				if (q == null || q.length() == 0) {
					showToast(getString(R.string.input_mobile_no_desc));
				} else {
					queryData(q);
				}
			}
	...
	Handler hander = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			switch (msg.what) {
			case QUERY_MSG:
				PhoneInfo pi = (PhoneInfo) msg.obj;
				String ret = getString(R.string.query_mobile_result_template,pi.getPhonenum(), pi.getLocation());
