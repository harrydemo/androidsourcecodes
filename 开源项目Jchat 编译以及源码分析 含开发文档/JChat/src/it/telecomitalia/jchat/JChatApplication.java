...
/**
 * This class is accessibile directly by all activities and it is used to get
 * stored preferences and useful data throughout the code
 * 这个类是直接accessibile所有活动，它是用来存储的喜好和有用的数据，整个代码
 */
...
public class JChatApplication extends Application
{
	...
	/**
	 * Retrieves a string as a replacement for the phone number if the phone
	 * number is not available (no <code>/data/local.prop</code> on emulator).
	 * 撷取一串更换电话号码，如果电话号码是不可用的（没有的<code>/数据/ local.prop的</代码>模拟器）。
