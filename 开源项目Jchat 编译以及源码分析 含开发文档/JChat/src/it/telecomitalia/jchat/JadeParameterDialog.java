...
/**
 * Custom dialog employed to show to the user current host and port for JADE
 * main container making him able to change default settings (read from
 * strings.xml) if he wishes to.
 * 自定义对话框来显示当前用户的主机和端口桥接主要集装箱他，
 * 如果他愿意，可以更改默认设置的strings.xml读。
 */
...
public class JadeParameterDialog extends Dialog
{
	...
	/**
	 * Initializes the dialog UI, preparing the parent view containing view
	 * hierarchy Layout is hardcoded here, no xml.
	 * 初始化对话框的用户界面，准备父视图包含视图层次布局是硬编码在这里，没有XML。
	 * @param ctx
	 *            the application context
	...
	private View initUI()
	{
		...
		ArrayAdapter<String> adapter = new ArrayAdapter<String>(activity, android.R.layout.simple_spinner_item, new String[]
