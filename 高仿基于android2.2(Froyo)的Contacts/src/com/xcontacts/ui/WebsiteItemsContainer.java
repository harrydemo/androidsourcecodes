...
public class WebsiteItemsContainer extends AbstractItemsContainer {

	public WebsiteItemsContainer(Context context) {
		super(context);
	}
	...
	public ArrayList<ContentProviderOperation> getItemsContentProviderOperation() {
		...
		if (count > 0) {
			...
			for (int i = 0; i < count; i++) {
				WebsiteItem item = (WebsiteItem) mItemsContainer.getChildAt(i);
				if (EditContactActivity.isInsert()) {// 新建联系人
					...
					// Website的默认Type是Website.TYPE_OTHER
	...
	public void addItem(int typesRes, int hintArrayRes) {
		WebsiteItem item = new WebsiteItem(mContext);
